    ...
    namespace System
    {
        ...
        public:
            inline void lock()
            {
                ...
                while(!TryLock())
                {
#if !defined(DS_PLATFORM_MACOS) && !defined(DS_PLATFORM_IOS)
                    if(spin < 10)
                    {
#ifdef DS_PLATFORM_WINDOWS
                        _mm_pause(); 
 #else
                        SYS_pause; // SMT thread swap can occur here
#endif
                    }
                    else
                    {
        ...
        namespace JobSystem
        {
            ...
            void init(uint32_t reservedThreads)
            {
                DS_PROFILE_FUNCTION();
                ...
                internal_state->numThreads = std::max<uint32_t>(1u, internal_state->numCores - reservedThreads);
                ...
                for(uint32_t threadID = 0; threadID < internal_state->numThreads; ++threadID)
                {
                    std::thread& worker = internal_state->threads.EmplaceBack([threadID]
                                ...
                                DS_PROFILE_SETTHREADNAME((const char*)name.str);
                                SetThreadName(name);
...
#define handle_error_en(en, msg) \
    do {                         \
        ...
        perror(msg);             \
                ...
                DS_LOG_INFO("Initialised JobSystem with [{0} cores] [{1} threads]", internal_state->numCores, internal_state->numThreads);
            }
