...
void main(uint2 px: SV_DispatchThreadID) {
    ...
    if (depth_tex[px] == FARZ) {
        float4 pos_cs = float4(uv_to_cs(uv), FARZ, 1.0);
        float4 pos_vs = mul(pos_cs, frame_constants.view_constants.clip_to_view);

        float4 prev_vs = pos_vs;

        float4 prev_cs = mul(prev_vs, frame_constants.view_constants.view_to_clip);
        float4 prev_pcs = mul(prev_cs,frame_constants.view_constants.clip_to_prev_clip);
    ...
    float3 eye_pos = mul(float4(0, 0, 0, 1),frame_constants.view_constants.view_to_world).xyz;
    ...
    float3 normal_pvs = mul(mul(mul(float4(normal_vs, 0), frame_constants.view_constants.view_to_clip),
        frame_constants.view_constants.clip_to_prev_clip),
        frame_constants.view_constants.prev_clip_to_prev_view).xyz;
    ...
    float4 prev_pvs = mul(prev_pcs,frame_constants.view_constants.prev_clip_to_prev_view);
    prev_pvs /= prev_pvs.w;
    ...
    float plane_dist_prev_dz = min(-0.02, normal_vs.z);
    ...
    float4 prev_view_z = rcp(prev_depth * -frame_constants.view_constants.prev_clip_to_prev_view._34);
