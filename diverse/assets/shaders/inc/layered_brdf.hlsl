#ifndef _LAYERED_BRDF_HLSL_
#define _LAYERED_BRDF_HLSL_
...
struct LayeredBrdf {
    ...
    float3 evaluate_pdf(float3 wo, float3 wi,inout float pdf) {
        if (wo.z <= 0 || wi.z <= 0) {
            return 0;
        }
        ...
        pdf = lerp(diff.pdf, spec.pdf, transmission_p);
        return (
            spec.value * energy_preservation.preintegrated_reflection_mult +
            diff.value * spec.transmission_fraction
        );
    ...
    void eval_diff_spec(float3 wo, float3 wi, out float3 out_diff, out float3 out_spec) {
        if (wo.z <= 0 || wi.z <= 0) {
            out_diff = 0.0;
            out_spec = 0.0;
            return ;
        ...
        out_spec = spec.value * energy_preservation.preintegrated_reflection_mult;
        out_diff = diff.value * spec.transmission_fraction;
    }
 ...
 */
float sample_lambert_pdf(float dotNL)
{
    return saturate(dotNL) * M_FRAC_1_PI; // PDF for the upper hemisphere.
}

float sample_brdf_pdf(LayeredBrdf brdf, float3 wo,float3 wi)
{
    float pdf = 1.0;
