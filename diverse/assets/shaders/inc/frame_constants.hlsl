...
struct FrameConstants {
    ...
    float4 sky_ambient; //w store intensity

    float pre_exposure;
    float pre_exposure_prev;
    float pre_exposure_delta;
    uint scene_lights_count;
...
static const float FARZ = 0.0f;
#else
static const float NEARZ = 0.0f;
...
struct ViewRayContext {
    ...
    static ViewRayContext from_uv(float2 uv) {
        ...
        ViewRayContext res;
        res.ray_dir_cs = float4(uv_to_cs(uv), FARZ, 1.0);
        res.ray_dir_vs_h = mul(res.ray_dir_cs, view_constants.sample_to_view);
        res.ray_dir_ws_h = mul(res.ray_dir_vs_h, view_constants.view_to_world);

        res.ray_origin_cs = float4(uv_to_cs(uv), NEARZ, 1.0);
        res.ray_origin_vs_h = mul(res.ray_origin_cs, view_constants.sample_to_view);
        res.ray_origin_ws_h = mul(res.ray_origin_vs_h,view_constants.view_to_world);
    ...
    static ViewRayContext from_uv_and_depth(float2 uv, float depth) {
        ...
        res.ray_hit_vs_h = mul(res.ray_hit_cs, view_constants.sample_to_view);
        res.ray_hit_ws_h = mul(res.ray_hit_vs_h, view_constants.view_to_world);
...
float3 get_eye_position() {
    float4 eye_pos_h = mul(float4(0, 0, 0, 1), frame_constants.view_constants.view_to_world);
    return eye_pos_h.xyz / eye_pos_h.w;
}

float3 get_prev_eye_position() {
    float4 eye_pos_h = mul(float4(0, 0, 0, 1), frame_constants.view_constants.prev_view_to_prev_world);
    return eye_pos_h.xyz / eye_pos_h.w;
}

float depth_to_view_z(float depth) {
    return rcp(depth * -frame_constants.view_constants.clip_to_view._34);
}

float3 direction_view_to_world(float3 v) {
    return mul(float4(v, 0), frame_constants.view_constants.view_to_world).xyz;
}

float3 direction_world_to_view(float3 v) {
    return mul(float4(v, 0),frame_constants.view_constants.world_to_view).xyz;
}

float3 position_world_to_view(float3 v) {
    return mul(float4(v, 1),frame_constants.view_constants.world_to_view).xyz;
}

float3 position_view_to_world(float3 v) {
    return mul(float4(v, 1),frame_constants.view_constants.view_to_world).xyz;
}

float3 position_world_to_clip(float3 v) {
    float4 p = mul(float4(v, 1), frame_constants.view_constants.world_to_view);
    p = mul(p, frame_constants.view_constants.view_to_clip);
    return p.xyz / p.w;
}

float3 position_world_to_sample(float3 v) {
    float4 p = mul(float4(v, 1), frame_constants.view_constants.world_to_view);
    p = mul(p, frame_constants.view_constants.view_to_sample);
    return p.xyz / p.w;
}
...
uint getBitValue(uint value, uint bit) {
    uint mask = 1u << bit;
    uint bitValue = (value & mask) ? 1 : 0;
    return bitValue;
}

uint getLowBitsValue(uint value, uint bit) {
    uint mask = (1U << bit) - 1;
    uint lowBitsValue = value & mask;
    return lowBitsValue;
}

int getTwoBitValue(int number, int i) {
    if (i < 0 || i > 30) {
        return -1;
    }
    int mask = 3u << i;
...
int setTwoBitValue(int number, int i, int value) {
    if (i < 0 || i > 30) {
        return number;
    }
    if (value < 0 || value > 3) {
