...
struct StandardBSDF
{
    ...
    {
        return diffuse_weight + specular_weight + transmission_weight + fuzz_weight + coat_weight;
    }
    ...
    BsdfSampleResult sample_diffuse_lambert(float3 wo, float2 urand)
    {
        ...
        result.value = result.lobe_value / result.pdf;
    ...
    BsdfLobeResult evaluate_specular_ggx(BsdfEvalData data)
    {
        ...
        float g1_wo = 2.0 / (1.0 + sqrt(max(0.0, 1.0 - (alpha2 * (1.0 - data.ndotv * data.ndotv)) / (data.ndotv * data.ndotv))));
        float g1_wi = 2.0 / (1.0 + sqrt(max(0.0, 1.0 - (alpha2 * (1.0 - data.ndotl * data.ndotl)) / (data.ndotl * data.ndotl))));
        float g = g1_wo * g1_wi;
        ...
        // Combined BRDF
        result.value = d * g * f / (4.0 * data.ndotv * data.ndotl);

        // PDF with VNDF sampling
        float g1 = g1_wo;
        result.pdf = g1 * d * vdoth / data.ndotv;
    ...
    BsdfSampleResult sample_specular_ggx_vndf(float3 wo, float2 urand)
    {
        ...
        // Compute PDF
        float alpha2 = alpha * alpha;
        float g1_wo = 2.0 / (1.0 + sqrt(max(0.0, 1.0 - (alpha2 * (1.0 - wo.z * wo.z)) / (wo.z * wo.z))));
        float d = alpha2 / (M_PI * pow(max(0.0, h.z * h.z * (alpha2 - 1.0) + 1.0), 2.0));
        float vdoth = saturate(dot(wo, h));
        result.pdf = g1_wo * d * vdoth / wo.z;
    ...
    BsdfLobeResult evaluate_fuzz_charlie(BsdfEvalData data)
    {
        ...
        result.value = fuzz_weight * sheen * view_factor * light_factor * M_FRAC_1_PI;
        result.pdf = data.ndotl * M_FRAC_1_PI;
    ...
    BsdfSampleResult sample_fuzz_charlie(float3 wo, float2 urand)
    {
        ...
        float phi = urand.x * M_TAU;
        float cos_theta = pow(max(0.0, 1.0 - urand.y), 1.0 / (fuzz_roughness + 1.0));
        float sin_theta = sqrt(max(0.0, 1.0 - cos_theta * cos_theta));

        result.wi = float3(cos(phi) * sin_theta, sin(phi) * sin_theta, cos_theta);
        result.pdf = cos_theta * M_FRAC_1_PI;
        result.lobe_value = fuzz_weight * pow(cos_theta, fuzz_roughness) * M_FRAC_1_PI;
        result.value = result.lobe_value / result.pdf;
    ...
    BsdfEvalResult evaluate(BsdfEvalData data)
    {
        ...
        if (has_lobe(LOBE_DIFFUSE_REFLECTION))
        {
            ...
            result.pdf += diffuse.pdf * diffuse_weight;
        }
        ...
        if (has_lobe(LOBE_SPECULAR_REFLECTION))
        {
            ...
            result.pdf += specular.pdf * specular_weight;
        }
        ...
        if (has_lobe(LOBE_FUZZ))
        {
            ...
            result.pdf += fuzz.pdf * fuzz_weight;
        }
    ...
    BsdfSampleResult sample(float3 wo, float3 urand)
    {
        ...
        float diffuse_p = diffuse_weight / total_weight;
        float specular_p = specular_weight / total_weight;
        float fuzz_p = fuzz_weight / total_weight;

        BsdfSampleResult result;
        float lobe_xi = urand.z;

        if (lobe_xi < diffuse_p && has_lobe(LOBE_DIFFUSE_REFLECTION))
        {
            result = sample_diffuse_lambert(wo, urand.xy);
            result.pdf *= diffuse_p;
            result.value /= diffuse_p;
        }
        else if (lobe_xi < diffuse_p + specular_p && has_lobe(LOBE_SPECULAR_REFLECTION))
        {
            result = sample_specular_ggx_vndf(wo, urand.xy);
            result.pdf *= specular_p;
            result.value /= specular_p;
        }
        else if (has_lobe(LOBE_FUZZ))
        {
