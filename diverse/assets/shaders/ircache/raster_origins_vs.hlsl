...
VsOut main(uint vid: SV_VertexID, uint instance_index: SV_InstanceID) {
    ...
    if (is_ircache_entry_life_valid(ircache_life_buf[entry_idx])) {
        ...
        const float4 vs_pos = mul(float4(ws_pos, 1.0), frame_constants.view_constants.world_to_view);
        const float4 cs_pos = mul(vs_pos, frame_constants.view_constants.view_to_sample);
