...
IrcacheLookup ircache_trilinear_lookup(float3 pt_ws, float3 normal_ws, float3 jitter) {
    const float3 eye_pos = get_eye_position();

    const int4 grid_coord = ws_pos_to_grid_coord(pt_ws.xyz, normal_ws, jitter);

    const uint cascade = grid_coord.w;

    const float3 center_cell_offset = pt_ws - irache_grid_center(grid_coord, eye_pos);
    const float cell_diameter = ircache_grid_cell_diameter_in_cascade(cascade);
    const int3 c0 = grid_coord.xyz + select(center_cell_offset > 0.0.xxx, (0).xxx, (-1).xxx);
    const float3 interp_t0 = select(center_cell_offset > 0.0.xxx, (0.0).xxx, (1.0).xxx);
        ...
        for (int y = 0; y < 2; ++y) {
            for (int x = 0; x < 2; ++x) {
                ...
                const uint4 sample_c4 = uint4(
                    clamp(sample_within_cascade, (int3)0, (int3)(IRCACHE_CASCADE_SIZE - 1)),
                    cascade);
    
                const uint cell_idx = irache_coord_to_cell_idx(sample_c4.xyz, sample_c4.w);
                ...
                if (cell_meta.y & IRCACHE_ENTRY_META_OCCUPIED) {
                    ...
                    interp = saturate( select( int3(x, y, z) == 0 , (1).xxx - interp ,interp ));
...
IrcacheLookup ircache_nearest_lookup(float3 pt_ws, float3 normal_ws, float3 jitter) {
    IrcacheLookup result;
    result.count = 0;
...
#if IRCACHE_USE_TRILINEAR  
#define ircache_lookup  ircache_trilinear_lookup
    static const uint IRCACHE_LOOKUP_MAX = 8;
#else
#define ircache_lookup  ircache_nearest_lookup
    static const uint IRCACHE_LOOKUP_MAX = 1;
#endif
...
IrcacheLookupMaybeAllocate IrcacheLookupParams::lookup_maybe_allocate(inout uint rng) {
    ...
    if (!IRCACHE_FREEZE) {
        ...
        const int3 was_just_scrolled_in =
            select(scroll_offset > 0,(int3(rcoord.coord) + scroll_offset >= IRCACHE_CASCADE_SIZE),(int3(rcoord.coord) < -scroll_offset));
