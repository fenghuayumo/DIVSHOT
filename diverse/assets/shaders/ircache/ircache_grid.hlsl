...
uint irache_coord_to_cell_idx( uint3 coord, uint cascade){
    return dot(
        uint4( coord, cascade),
        uint4(
            1,
...
float3 irache_grid_center(uint4 coord, float3 eye_pos) {
    // const int3 cascade_origin = frame_constants.ircache_cascades[cascade].origin.xyz;
    eye_pos = trunc(eye_pos / IRCACHE_GRID_CELL_DIAMETER) * IRCACHE_GRID_CELL_DIAMETER;
    
    return eye_pos + ((coord.xyz + 0.5.xxx - IRCACHE_CASCADE_SIZE / 2) * IRCACHE_GRID_CELL_DIAMETER) * (1u << uint(coord.w));
}

uint4 ws_pos_to_grid_coord(float3 pos, float3 normal, float3 jitter) {
    ...
    return uint4(uint3(clamp(coord, (0).xxx, (IRCACHE_CASCADE_SIZE-1).xxx)),
            cascade);
}
...
IrcacheCoord ws_pos_to_ircache_coord(float3 pos, float3 normal, float3 jitter) {
    ...
    {
        uint cascade = ws_local_pos_to_cascade_idx(pos - center, reserved_cells);
    #ifdef USE_DEFINE_CASCADE
        cascade += 2;
    #endif
        const float cell_diameter = (IRCACHE_GRID_CELL_DIAMETER * (1u << cascade));
        float Scale = 1.0;
    #if SCALE_CELL_DIAMETER
        Scale = 4.0;
    #endif
        pos += cell_diameter * jitter * Scale;
    }
