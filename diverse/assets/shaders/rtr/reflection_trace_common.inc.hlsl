        ...
        if (primary_hit.is_hit) {
            ...
            float3 reflected_normal_vs;
            {
                ...
                if (USE_SCREEN_GI_REPROJECTION && is_on_screen) {
                    ...
                    // if( USE_DDGI ){
                    //     const float3 new_wo = normalize(outgoing_ray.Origin - primary_hit.position);
                    //     total_radiance += gbuffer.albedo * sample_irradiance(ddgi, primary_hit.position.xyz, gbuffer.normal, new_wo, s_Irradiance, s_Depth);
                    // }
               }
            }
