...
void main(uint2 px: SV_DispatchThreadID) {
    ...
    const float4 reflection_hit_cs = mul(float4(reflection_hit_vs, 1), frame_constants.view_constants.view_to_sample);
    const float4 prev_hit_cs = mul(reflection_hit_cs,frame_constants.view_constants.clip_to_prev_clip);
    float2 hit_prev_uv = cs_to_uv(prev_hit_cs.xy / prev_hit_cs.w);

    const float4 prev_reflector_cs = mul(view_ray_context.ray_hit_cs,frame_constants.view_constants.clip_to_prev_clip);
