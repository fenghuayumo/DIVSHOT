...
DS_RESOURCE(2) RWStructuredBuffer<uint> g_InputValues;
DS_RESOURCE(3) RWStructuredBuffer<uint> g_OutputValues;
DS_RESOURCE(4) RWStructuredBuffer<uint> g_GroupHistograms;
#if USE_ARGS
DS_RESOURCE(5) RWStructuredBuffer<uint4> g_CountBuffer;
          ...
          in uint bidx: SV_GroupID)
{
    ...
    for (uint i = 0; i < KEYS_PER_THREAD; ++i)
    {
        ...
        uint key = (key_index < GetCount() ? g_InputKeys[key_index + GetOffset()] : 0xFFFFFFFFu);
#ifdef SORT_VALUES
        uint value = (key_index < GetCount() ? g_InputValues[key_index + GetOffset()] : 0);
#endif
        ...
        {
            g_OutputKeys[global_offset + local_offset + GetOffset()] = key;
#ifdef SORT_VALUES
            g_OutputValues[global_offset + local_offset + GetOffset()] = value;
#endif
        }
        GroupMemoryBarrierWithGroupSync();
