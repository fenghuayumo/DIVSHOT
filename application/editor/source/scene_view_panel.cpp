...
namespace diverse
{
    ...
    SceneViewPanel::SceneViewPanel(bool active)
        :EditorPanel(active)
    {
        m_Name = U8CStr2CStr(ICON_MDI_GAMEPAD_VARIANT " Scene###scene");
        m_SimpleName = "Scene";
    ...
    void SceneViewPanel::on_imgui_render()
    {
        ...
        maths::Transform* transform = nullptr;
        {
            ...
            camera = m_Editor->get_camera();
            transform = &m_Editor->get_editor_camera_transform();
        ...
        m_Editor->scene_view_panel_position = glm::vec2(sceneViewPosition.x, sceneViewPosition.y);

        bool resized = resize(static_cast<uint32_t>(sceneViewSize.x), static_cast<uint32_t>(sceneViewSize.y));

        auto game_view_tex = m_Editor->get_main_render_texture();
        ...
        m_Editor->set_scene_view_active(updateCamera);
        m_Editor->set_scene_view_rect(maths::Rect(sceneViewPosition, sceneViewSize));
        {
        ...
        if(m_Editor->get_current_train_view_image())
        {
            ...
            auto train_view_tex = m_Editor->get_current_train_view_image()->gpu_texture;
        ...
        if (m_Editor->show_grid())
        {
            if (camera->is_orthographic())
            {
                DS_PROFILE_SCOPE("2D Grid");
                m_Editor->draw_2dgrid(ImGui::GetWindowDrawList(), { transform->get_world_position().x, transform->get_world_position().y }, sceneViewPosition, { sceneViewSize.x, sceneViewSize.y }, camera->get_scale(), 1.5f);    
            }
        }
      
        m_Editor->imguizmo(ImGui::GetWindowDrawList(), { transform->get_world_position().x, transform->get_world_position().y }, sceneViewPosition, { sceneViewSize.x, sceneViewSize.y });
        //draw splat crop
        drwa_splat_mask_box(camera, transform, sceneViewPosition, sceneViewSize);
        ...
        if (ImGui::IsWindowFocused() && updateCamera && app.get_scene_active() && !ImGuizmo::IsUsing() && ImGui::IsMouseClicked(ImGuiMouseButton_Left) && valid_region)
        {
            ...
            auto old_pivot_transform = m_Editor->get_pivot()->get_transform();
            ...
            maths::Ray ray = m_Editor->get_screen_ray(int(clickPos.x), int(clickPos.y), camera, view_size.x, view_size.y);
            m_Editor->select_object(ray);

             auto splat_ent = Entity(m_Editor->get_current_splat_entt(), m_Editor->get_current_scene());
             if (splat_ent.valid() && splat_ent.active() && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left))
             {
                 ...
                 maths::Transform& cameraTransform = m_Editor->get_editor_camera_transform();
                 ...
                 if (pick_splat(splat_model,
                     ...
                     isect_point))
                 {
                     auto& pivot_transform = m_Editor->get_pivot()->get_transform();
                     pivot_transform = transform;
                     pivot_transform.set_local_position(isect_point);
                     pivot_transform.set_world_matrix(glm::mat4(1.0f));
                     GaussianEdit::get().add_place_pivot_op(old_pivot_transform, pivot_transform, m_Editor->get_pivot());
                 }
             }
        }
        if (ImGui::IsWindowFocused() && updateCamera && !ImGuizmo::IsUsing() && ImGui::IsItemHovered() && Input::get().get_mouse_mode() == MouseMode::Visible)
        {
            ...
            maths::Ray ray = m_Editor->get_screen_ray(int(clickPos.x), int(clickPos.y), camera, int(sceneViewSize.x), int(sceneViewSize.y));
            m_Editor->select_object(ray, true);
        }
        else
        {
            m_Editor->set_hovered_entity(entt::null);
            m_Editor->set_hovered_train_view(-1);
        }
        if (m_Editor->get_hovered_train_view())
        {
            ImGuiHelper::ScopedStyle scopedStyle(ImGuiStyleVar_WindowPadding, ImVec2(4, 4));
            ImGui::BeginTooltip();
            ImGui::TextUnformatted(m_Editor->get_hovered_train_view()->file_path.c_str());
            ImGui::EndTooltip();
        }
        ...
        if (ImGui::BeginDragDropTarget())
        {
            ...
            if (data)
            {
                ...
                m_Editor->file_open_callback(file);
            }
    ...
    void SceneViewPanel::toolbar()
    {
        ...
        {
            selected = m_Editor->get_imguizmo_operation() == ImGuizmo::TRANSLATE;
            if (selected)
                ImGui::PushStyleColor(ImGuiCol_Text, ImGuiHelper::GetSelectedColour());
            ImGui::SameLine();
            if (ImGui::Button(U8CStr2CStr(ICON_MDI_ARROW_ALL)))
                m_Editor->set_imguizmo_operation(ImGuizmo::TRANSLATE);
        ...
        {
            selected = m_Editor->get_imguizmo_operation() == ImGuizmo::ROTATE;
            if (selected)
                ImGui::PushStyleColor(ImGuiCol_Text, ImGuiHelper::GetSelectedColour());

            ImGui::SameLine();
            if (ImGui::Button(U8CStr2CStr(ICON_MDI_ROTATE_ORBIT)))
                m_Editor->set_imguizmo_operation(ImGuizmo::ROTATE);
        ...
        {
            selected = m_Editor->get_imguizmo_operation() == ImGuizmo::SCALE;
            if (selected)
                ImGui::PushStyleColor(ImGuiCol_Text, ImGuiHelper::GetSelectedColour());

            ImGui::SameLine();
            if (ImGui::Button(U8CStr2CStr(ICON_MDI_ARROW_EXPAND_ALL)))
                m_Editor->set_imguizmo_operation(ImGuizmo::SCALE);
        ...
        {
            selected = m_Editor->get_imguizmo_operation() == ImGuizmo::UNIVERSAL;
            if (selected)
                ImGui::PushStyleColor(ImGuiCol_Text, ImGuiHelper::GetSelectedColour());

            ImGui::SameLine();
            if (ImGui::Button(U8CStr2CStr(ICON_MDI_CROP_ROTATE)))
                m_Editor->set_imguizmo_operation(ImGuizmo::UNIVERSAL);
        ...
        {
            selected = m_Editor->get_imguizmo_operation() == ImGuizmo::BOUNDS;
            if (selected)
                ImGui::PushStyleColor(ImGuiCol_Text, ImGuiHelper::GetSelectedColour());

            ImGui::SameLine();
            if (ImGui::Button(U8CStr2CStr(ICON_MDI_BORDER_NONE)))
                m_Editor->set_imguizmo_operation(ImGuizmo::BOUNDS);
        ...
        {
           selected = (m_Editor->snap_guizmo() == true);

           if (selected)
               ImGui::PushStyleColor(ImGuiCol_Text, ImGuiHelper::GetSelectedColour());

           if (ImGui::Button(U8CStr2CStr(ICON_MDI_MAGNET)))
               m_Editor->snap_guizmo() = !selected;
        ...
        if (ImGui::BeginPopup("GizmosPopup"))
        {
...
#endif
            ImGui::Checkbox("Grid", &m_Editor->show_grid());
            ImGui::Checkbox("Selected Gizmos", &m_Editor->show_gizmos());
            ImGui::Checkbox("View Selected", &m_Editor->show_view_selected());

            ImGui::Separator();
            ImGui::Checkbox("Camera", &m_ShowComponentGizmoMap[typeid(Camera).hash_code()]);
            
            ImGui::Separator();
            uint32_t flags = m_Editor->get_settings().debug_draw_flags;
            ...
            m_Editor->get_settings().debug_draw_flags = flags;
            ImGui::Separator();
        ...
        {
            auto& camera = *m_Editor->get_camera();
            ...
            if (ImGui::Button(U8CStr2CStr(ICON_MDI_AXIS_ARROW " 3D")))
            {
                ...
                m_Editor->get_editor_camera_controller().set_current_mode(EditorCameraMode::ARCBALL);
            }
            if (selected)
                ImGui::PopStyleColor();
            ImGui::NextColumn();
            ...
            if (ImGui::Button(U8CStr2CStr(ICON_MDI_ANGLE_RIGHT "2D")))
            {
                if (!ortho)
                {
                    ...
                    // auto camPos = m_Editor->get_editor_camera_transform().get_local_position();
                    // camPos.z = 0.0f;
                    camera.set_near(-10.0f);
                    // m_Editor->get_editor_camera_transform().set_local_position(camPos);
                    // m_Editor->get_editor_camera_transform().set_local_orientation(glm::quat(glm::vec3(0.0f, 0.0f, 0.0f)));
                    // m_Editor->get_editor_camera_transform().set_local_scale(glm::vec3(1.0f, 1.0f, 1.0f));

                    m_Editor->get_editor_camera_controller().set_current_mode(EditorCameraMode::TWODIM);
                }
            }
            if (selected)
                ImGui::PopStyleColor();
            ...
            if (!ortho)
            {
                auto mode = m_Editor->get_editor_camera_controller().get_current_mode();
                selected = mode == EditorCameraMode::FLYCAM;
                if (selected)
                    ImGui::PushStyleColor(ImGuiCol_Text, ImGuiHelper::GetSelectedColour());
                if (ImGui::Button("Fly"))
                    m_Editor->get_editor_camera_controller().set_current_mode(EditorCameraMode::FLYCAM);
                if (selected)
                    ImGui::PopStyleColor();
        ...
        /*    ImGui::SliderInt("Width", (int*) & m_Width, 128,2048);
            ...
            auto fov = m_Editor->get_camera()->get_fov();
            if (ImGui::DragFloat("Fov",&fov,1.0f,0, 90.0f))
                m_Editor->get_camera()->set_fov(fov);
            auto speed = m_Editor->get_editor_camera_controller().get_speed();
            if (ImGui::DragFloat("Speed", &speed,1.0f, 0, 100.0f))
                m_Editor->get_editor_camera_controller().set_speed(speed);
            float Near = m_Editor->get_camera()->get_near();
            if (ImGui::DragFloat("Near", &Near))
                m_Editor->get_camera()->set_near(Near);
            float Far = m_Editor->get_camera()->get_far();
            if (ImGui::DragFloat("Far", &Far))
                m_Editor->get_camera()->set_far(Far);
#ifndef DS_PRODUCTION
            auto rot = glm::degrees(m_Editor->get_editor_camera_transform().get_local_orientation());
            if(ImGui::DragFloat3("rot", glm::value_ptr(rot)) )
            {
                m_Editor->get_editor_camera_transform().set_local_orientation(glm::radians(rot));
                m_Editor->get_editor_camera_transform().set_world_matrix(glm::mat4(1.0f));
            }
            auto pos = m_Editor->get_editor_camera_transform().get_local_position();
            if (ImGui::DragFloat3("pos", glm::value_ptr(pos)))
            {
                m_Editor->get_editor_camera_transform().set_local_position(pos);
                m_Editor->get_editor_camera_transform().set_world_matrix(glm::mat4(1.0f));
            }
#endif
            ImGui::EndPopup();
        ...
        {   
            ...
            if(ImGui::Button(U8CStr2CStr("  Edit" ICON_MDI_TOOLTIP_EDIT)))
                m_Editor->splat_edit() = !m_Editor->splat_edit();
            ImGui::EndPopup();
        }
        ImGui::SameLine();
        ImGui::SeparatorEx(ImGuiSeparatorFlags_Vertical);
    ...
    {
        DS_PROFILE_FUNCTION();
        //m_Editor->GetSettings().aspect_ratio = 1.0f;
        m_CurrentScene = scene;
        Application::get().set_override_camera(m_Editor->get_camera(), &m_Editor->get_editor_camera_transform());
    }

    void SceneViewPanel::draw_gizmos(float width, float height, float xpos, float ypos, Scene* scene)
    {
        DS_PROFILE_FUNCTION();
        Camera* camera = m_Editor->get_camera();
        maths::Transform& cameraTransform = m_Editor->get_editor_camera_transform();
    ...
    bool SceneViewPanel::resize(uint32_t width, uint32_t height)
    {
        ...
        if(resize)
        {
            m_Editor->handle_renderer_resize(m_Width,m_Height);
        }
        return resize;
    }
        ...
        { 
            ...
            { 
                ...
                if (gaussian)
                {
                    ...
                    splatEdit.set_edit_scene(m_Editor->get_current_scene());
                }
            ...
            {
                auto& gaussian = reg.get<diverse::GaussianComponent>(m_Editor->get_current_splat_entt());
                auto& gs_transform = reg.get<diverse::maths::Transform>(m_Editor->get_current_splat_entt());
            ...
            }
            {
                ...
                if (ImGui::BeginPopup("SplatSetting"))
                {
                    ...
                    ImGui::Checkbox("Histogram", &m_Editor->histogram_panel->active());
    ...
    void SceneViewPanel::draw_progress(const ImVec2& sceneViewPosition, const ImVec2& sceneViewSize)
    {
        ...
        auto train_ent = m_CurrentScene->get_entity_manager()->get_entities_with_type<GaussianTrainerScene>();
        if (!train_ent.empty())
        {
        ...
        auto mesh_group = m_CurrentScene->get_registry().group<MeshModelComponent>(entt::get<maths::Transform>);
        for (auto ent : mesh_group)
        {
            if (!Entity(ent, m_CurrentScene).active())
                continue;
        ...
        //point
        auto point_group = m_CurrentScene->get_registry().group<PointCloudComponent>(entt::get<maths::Transform>);
        ...
        auto group = m_CurrentScene->get_registry().group<GaussianComponent>(entt::get<maths::Transform>);
        for (auto gs_ent : group)
        {
            if (!Entity(gs_ent, m_CurrentScene).active())
                continue;
    ...
    {
        if (!m_Editor->splat_edit()) return;
        auto updateCamera = m_Editor->get_scene_view_active();
        if ( ImGui::IsWindowFocused())
        {
            auto& gs_edit = GaussianEdit::get();
            if(!m_Editor->splat_edit())  gs_edit.set_edit_type(diverse::GaussianEdit::EditType::None);
            gs_edit.create_brush_buffer(m_Width, m_Height);
            ...
            if (gs_edit.get_edit_type() != GaussianEdit::EditType::None)
            {
                ...
                { 
                    ...
                    {
                        m_StartMousePos = ImGui::GetMousePos();//Input::get().get_mouse_position();
                        if (gs_edit.get_edit_type() == GaussianEdit::EditType::Rect)
                        {
                            auto p0 = m_StartMousePos - sceneViewPosition;
                    ...
                    else if(updateCamera)
                    {
                        ...
                        { 
                            ...
                            { 
                                ...
                                m_EndMousePos = ImGui::GetMousePos();//Input::get().get_mouse_position();
                            ...
                            {
                                if (gs_edit.get_edit_type() == GaussianEdit::EditType::Box || gs_edit.get_edit_type() == GaussianEdit::EditType::Sphere)
                                {
                                    gs_edit.reset_splat_intersected();
                                    gs_edit.intersect_splat(op);
                                }
                                else if(!ImGuizmo::IsUsing() && m_IsDoingSplatSelectOp)
                                { 
                                    gs_edit.reset_splat_intersected();
                                    gs_edit.intersect_splat(op);
                                }
                                m_IsDoingSplatSelectOp = false;
                            }
                        }
                    }
                    if (gs_edit.get_edit_type() == GaussianEdit::EditType::Polygon)
                ...
                auto model = gs_edit.get_edit_transform();
                ...
                else if (gs_edit.get_edit_type() == GaussianEdit::EditType::Sphere)
                {
                    ...
                    const auto scale = glm::compMax(model.get_local_scale());
    ...
    void SceneViewPanel::drwa_splat_mask_box(Camera* camera, maths::Transform* camera_transform,const ImVec2& sceneViewPosition, const ImVec2& sceneViewSize)
    {
        if(!m_Editor->enable_focus_region) return;
        const auto& bd_box = m_Editor->get_focus_splat_region();
        auto b = bd_box.max();
        auto a = bd_box.min();
        glm::mat4 view = glm::inverse(camera_transform->get_world_matrix());
        glm::mat4 proj = camera->get_projection_matrix();
        auto world2proj = proj * view;
        auto& model = m_Editor->focus_region_transform;
    ...
    {
        auto splat_group = m_CurrentScene->get_registry().group<GaussianCrop>(entt::get<maths::Transform>);
    ...
    void SceneViewPanel::draw_3d_paint_toolbar(const ImVec2& sceneViewPosition, const ImVec2& sceneViewSize)
    {
        ...
        {
            auto entitys = m_Editor->get_selected();
            auto size = item_btn_size + ImVec2(10, 10);
            auto offsetY = sceneViewPosition.y + (sceneViewSize.y - 12 * size.y) / 2.0f;
            ImGui::SetCursorPosY(offsetY);
            ...
            if (ImGui::Button(U8CStr2CStr(ICON_MDI_BRUSH), item_btn_size))
            {
                if (entitys.size() > 0)
                {
                    ...
                    auto& registry = m_Editor->get_current_scene()->get_registry();
    ...
    void SceneViewPanel::take_photo_dialog()
    {
        ...
        ImGuiHelper::TextCentred(m_photon_setting.file_path.c_str());
        ImGui::SameLine();
        if (ImGuiHelper::Button("..")) //open file dialog
        {
            m_photon_setting.file_path = FileDialogs::saveFile({ "jpg","png"});
        }
        ImGui::Separator();
        ...
        m_photon_setting.resolution = resolution[cur_res];

        ImGuiHelper::Property("Spp",m_photon_setting.spp);
        ImGui::Columns(1);
        ImGui::Separator();
        ImGui::PopStyleVar();
        ...
        if (ImGui::Button("OK", ImVec2(button_sizex, 0)) && !m_photon_setting.file_path.empty())
        {
            auto variable = CmadVariableMgr::get().find("r.takePhoton");
            if(variable) variable->set_value<bool>(true);
            variable = CmadVariableMgr::get().find("r.accumulate.spp");
            if(variable) variable->set_value<int>(m_photon_setting.spp);
            m_Editor->handle_renderer_resize(m_photon_setting.resolution.x, m_photon_setting.resolution.y);
            m_Editor->invalidate_pt_state();
    ...
    void SceneViewPanel::render_frame_image()
    {
        ...
        if (m_photon_setting.frame_counter == 0)
        {
            ImGui::OpenPopup("Render Image");
        }
        f32 progress = static_cast<f32>(m_photon_setting.frame_counter) / static_cast<f32>(m_photon_setting.spp);
        if (ImGuiHelper::ProgressWindow("Render Image", progress, ImVec2(240,0), true))
        {
            reset_render_frame();
        }
        if (++m_photon_setting.frame_counter % m_photon_setting.spp == 0)
        {
            ...
            write_stbi(m_photon_setting.file_path, res.x, res.y, 4, (uint8_t*)img_data.data(), 100);
            reset_render_frame();
        }
    }
    ...
    {
        m_photon_setting.frame_counter = 0;
        auto variable = CmadVariableMgr::get().find("r.takePhoton");
