...
PointSet *readPointSet(const std::string &filename) {
    ...
    else if(p.extension().string() == ".txt") r = colmapTextReadPointSet(filename);
...
PointSet *fastPlyReadPointSet(const std::string &filename) {
    ...
    checkHeader(reader, "x",float_type);
    checkHeader(reader, "y",float_type);
    checkHeader(reader, "z",float_type);
    ...
    if (ascii) {
        ...
        for (size_t i = 0; i < count; i++) {
            if(float_type){
                ...
                double x, y, z;
                reader >> x >> y >> z;
                r->points[i][0] = static_cast<float>(x);
                r->points[i][1] = static_cast<float>(y);
                r->points[i][2] = static_cast<float>(z);
            }
            if (hasNormals) {
        ...
        // Optimization: Use bulk read for simple binary PLY format (float xyz + uchar rgb)
        // This handles the most common point cloud format
        if (float_type && !hasNormals && hasColors && !hasViews) {
            // Define packed structure for bulk reading (cross-platform)
            #pragma pack(push, 1)
            struct PointColorData {
                float xyz[3];
                uint8_t rgb[3];
            };
            #pragma pack(pop)
            
            // Bulk read all data in one operation
            std::vector<PointColorData> buffer(count);
            reader.read(reinterpret_cast<char*>(buffer.data()), count * sizeof(PointColorData));
            
            // Fast copy to target structure
            for (size_t i = 0; i < count; i++) {
                r->points[i][0] = buffer[i].xyz[0];
                r->points[i][1] = buffer[i].xyz[1];
                r->points[i][2] = buffer[i].xyz[2];
                r->colors[i][redIdx] = buffer[i].rgb[0];
                r->colors[i][greenIdx] = buffer[i].rgb[1];
                r->colors[i][blueIdx] = buffer[i].rgb[2];
            }
        } else if (float_type && !hasNormals && !hasColors && !hasViews) {
            // Optimization: Direct bulk read for xyz-only point clouds
            reader.read(reinterpret_cast<char*>(r->points.data()), count * sizeof(float) * 3);
        } else {
            // Fallback to per-point reading for complex formats
            uint8_t color[3];
            ...
            for (size_t i = 0; i < count; i++) {
                if (float_type) {
                    ...
                    double x, y, z;
                    reader.read(reinterpret_cast<char *>(&x), sizeof(double));
                    reader.read(reinterpret_cast<char *>(&y), sizeof(double));
                    reader.read(reinterpret_cast<char *>(&z), sizeof(double));
                    r->points[i][0] = static_cast<float>(x);
                    r->points[i][1] = static_cast<float>(y);
                    r->points[i][2] = static_cast<float>(z);
                }
...
PointSet* colmapTextReadPointSet(const std::string& filename){
    auto lines = diverse::read_text_file(filename);
    uint64_t N = lines.size();
    auto* r = new PointSet();

    r->points.resize(N);
    r->colors.resize(N);
    auto& pos_data = r->points;
    auto& col_data = r->colors;
    ...
    for (uint64_t i = 0; i < N; ++i) {
        ...
        pos_data[i][0] = std::stof(tokens[1]);
        pos_data[i][1] = std::stof(tokens[2]);
        pos_data[i][2] = std::stof(tokens[3]);

        col_data[i][0] = std::stoi(tokens[4]);
        col_data[i][1] = std::stoi(tokens[5]);
        col_data[i][2] = std::stoi(tokens[6]);
    }
    return r;
}
void checkHeader(std::ifstream &reader, const std::string &prop,bool& float_type) {
    std::string line;
    std::getline(reader, line);
...
void fastPlySavePointSet(PointSet &pSet, const std::string &filename) {
    ...
    o << "comment Generated by diverseshot" << std::endl;
    ...
    // Optimization: Use bulk write for common formats
    const size_t count = pSet.count();
    
    if (!hasNormals && hasColors && !hasViews) {
        // Most common format: xyz + rgb - use bulk write
        #pragma pack(push, 1)
        struct PointColorData {
            float xyz[3];
            uint8_t rgb[3];
        ...
        std::vector<PointColorData> buffer(count);
        for (size_t i = 0; i < count; i++) {
            buffer[i].xyz[0] = pSet.points[i][0];
            buffer[i].xyz[1] = pSet.points[i][1];
            buffer[i].xyz[2] = pSet.points[i][2];
            buffer[i].rgb[0] = pSet.colors[i][0];
            buffer[i].rgb[1] = pSet.colors[i][1];
            buffer[i].rgb[2] = pSet.colors[i][2];
        }
        o.write(reinterpret_cast<const char*>(buffer.data()), count * sizeof(PointColorData));
        
    } else if (!hasNormals && !hasColors && !hasViews) {
        // xyz only - direct bulk write
        o.write(reinterpret_cast<const char*>(pSet.points.data()), count * sizeof(float) * 3);
        
    } else {
        // Complex format - fallback to per-point writing
        for (size_t i = 0; i < count; i++) {
