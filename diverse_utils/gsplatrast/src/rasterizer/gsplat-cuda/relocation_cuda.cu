...
namespace gsplat {
    ...
    inline __device__ scalar_t clamp(scalar_t value, scalar_t min_val, scalar_t max_val) {
        float clamped_value = fminf(fmaxf(value, min_val), max_val);
    ...
    __global__ void relocation_kernel(
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            // new_scales[idx * 3 + i] = clamp(new_scales[idx * 3 + i], 1e-4, 100.0f);
        }
    }
