...
__global__ void projection_ewa_3dgs_packed_fwd_kernel(
    ...
    scalar_t *__restrict__ t_opacities,    // [nnz] optional
    ...
    if (valid) {
        ...
        if (opacities != nullptr) {
            opacity = sigmoid(opacities[col_idx]);
    ...
    if (block_cnts != nullptr) {
        ...
        if (valid) {
            ...
            t_opacities[thread_data] = opacity;
            if (compensations != nullptr) {
...
void launch_projection_ewa_3dgs_packed_fwd_kernel(
    ...
    at::optional<at::Tensor> t_opacities, // [nnz] optional
                ...
                projection_ewa_3dgs_packed_fwd_kernel<scalar_t,true>
                    ...
                    at::cuda::getCurrentCUDAStream()>>>(
                        ...
                        compensations.has_value()
                            ...
                            : nullptr,
                        t_opacities.has_value() ? t_opacities.value().data_ptr<scalar_t>()
                                        : nullptr,
...
__global__ void projection_ewa_3dgs_packed_bwd_kernel(
    ...
    const scalar_t *__restrict__ v_t_opacities, // [nnz] optional
    ...
    const float sigmoid_opacities = sigmoid(opacities[gid]);
    ...
    if (v_compensations != nullptr) {
        ...
        const float v_compensation = v_t_opacity * sigmoid_opacities;
        add_blur_vjp(
    ...
    if constexpr (GEO) {
        float ts = v_ray_ts[0];
        v_mean_c += ts * glm::normalize(mean_c);
    }
...
#pragma unroll
        ...
        if (v_opacities != nullptr) {
            //in forward pass, t_opacities = sigmoid(opacitices) * compenstation, so we need to backpropagate the gradient of t_opacities to v_opacities
            v_opacities[idx] = v_t_opacity * sigmoid_opacities * (1 - sigmoid_opacities) * compensation;
        }
        if (v_covars != nullptr) {
            v_covars += idx * 6;
...
#pragma unroll
        ...
        if (v_opacities != nullptr) {
            //in forward pass, t_opacities = sigmoid(opacitices) * compenstation, so we need to backpropagate the gradient of t_opacities to v_opacities
            float v_opacity = v_t_opacity * sigmoid_opacities * (1 - sigmoid_opacities) * compensation;
...
void launch_projection_ewa_3dgs_packed_bwd_kernel(
    ...
    const at::Tensor& v_t_opacities, // [nnz] optional
    ...
    at::Tensor& v_opacities, // [N] optional
