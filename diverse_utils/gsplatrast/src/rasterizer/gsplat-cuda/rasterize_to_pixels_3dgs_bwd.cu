...
__global__ void rasterize_to_pixels_3dgs_bwd_kernel(
    ...
    const float *__restrict__ backgrounds, // [C, CDIM] or [nnz, CDIM]
    ...
    // grad outputs
    const float *__restrict__ v_render_colors, // [C, image_height,
                                                  // image_width, CDIM]
    const float
        *__restrict__ v_render_alphas, // [C, image_height, image_width, 1]
    const float* __restrict__ v_render_expected_depths, // [C, image_height, image_width, 1]
    const float* __restrict__ v_render_median_depths, // [C, image_height, image_width, 1]
    ...
    float *__restrict__ v_colors,   // [C, N, CDIM] or [nnz, CDIM]
    float *__restrict__ v_opacities, // [C, N] or [nnz]
    float* __restrict__ v_ray_ts,             // [C, N] or [nnz]
    float* __restrict__ v_ray_planes,   // [C, N, 2] or [nnz, 2]
...
void launch_rasterize_to_pixels_3dgs_bwd_kernel(
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_3dgs_bwd_kernel<CDIM,GEO>,
    ...
    rasterize_to_pixels_3dgs_bwd_kernel<CDIM,GEO>
        <<<grid, threads, shmem_size, at::cuda::getCurrentCUDAStream()>>>(
            ...
            GEO ? median_ids.value().data_ptr<int32_t>() : nullptr,
            v_render_colors.data_ptr<float>(),
            v_render_alphas.data_ptr<float>(),
            GEO ? v_render_expected_depths.value().data_ptr<float>() : nullptr,
            GEO ? v_render_median_depths.value().data_ptr<float>() : nullptr,
            GEO ? reinterpret_cast<vec3 *>(v_render_expected_normals.value().data_ptr<float>()) : nullptr,
            ...
            GEO ? v_ray_ts.value().data_ptr<float>() : nullptr,
            GEO ? v_ray_planes.value().data_ptr<float>() : nullptr,
            GEO ? v_normals.value().data_ptr<float>() : nullptr
        );
}
...
#define __INS__(CDIM,GEO)                                                          \
    template void launch_rasterize_to_pixels_3dgs_bwd_kernel<CDIM,GEO>(            \
        ...
        const at::optional<at::Tensor> median_id,                              \
        const at::Tensor v_render_colors,                                      \
        const at::Tensor v_render_alphas,                                      \
        const at::optional<at::Tensor> expected_depths,                        \
        const at::optional<at::Tensor> median_depths,                          \
        const at::optional<at::Tensor> expected_normals,                       \
