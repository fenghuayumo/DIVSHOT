...
namespace gsplat {
    ...
    inline __device__ uint32_t div_round_up(uint32_t value, uint32_t divisor) {
...
inline __device__ void persp_proj_vjp(
    ...
    // mat3x2 is 3 columns x 2 rows.
    mat3x2 J = mat3x2(fx * rz, 0.f,                  // 1st column
    ...
    if constexpr (GEO) {
        ...
        v_mean3d_ += vec3(fx * rz * v_mean2d[0], fy * rz * v_mean2d[1],
        ...
        v_mean3d += vec3(fx * rz * v_mean2d[0], fy * rz * v_mean2d[1],
            -(fx * x * v_mean2d[0] + fy * y * v_mean2d[1]) * rz2);
