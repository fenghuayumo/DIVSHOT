...
// Constexpr helpers.
constexpr inline Vec3f vec3f(const float *data) { return {data[0], data[1], data[2]}; }

constexpr inline float dot(const Vec3f &a, const Vec3f &b) {
  return a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
}

constexpr inline float squaredNorm(const Vec3f &v) { return dot(v, v); }

constexpr inline Quat4f quat4f(const float *data) { return {data[0], data[1], data[2], data[3]}; }

constexpr inline Vec3f times(const Quat4f &q, const Vec3f &p) {
  auto [w, x, y, z] = q;
  auto [vx, vy, vz] = p;
  auto x2 = x + x;
  auto y2 = y + y;
...
constexpr inline Quat4f plus(const Quat4f &a, const Quat4f &b) {
  return {a[0] + b[0], a[1] + b[1], a[2] + b[2], a[3] + b[3]};
}

constexpr inline Vec3f times(const Vec3f &v, float s) { return {v[0] * s, v[1] * s, v[2] * s}; }

constexpr inline Vec3f plus(const Vec3f &a, const Vec3f &b) {
  return {a[0] + b[0], a[1] + b[1], a[2] + b[2]};
}
