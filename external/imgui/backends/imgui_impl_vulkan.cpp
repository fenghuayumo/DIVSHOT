...
#include <stdio.h>
//#include <diverse/source/backend/drs_rhi/gpu_device.h>
...
static void ImGui_ImplVulkan_CreateDescriptorSetLayout(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ...
    std::vector<VkDescriptorBindingFlags> set_binding_flags(1, VkDescriptorBindingFlagBits::VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT | VkDescriptorBindingFlagBits::VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT);
    ...
    info.pNext = &binding_flags_create_info;
...
void    ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    if(g_device)
    {
        ...
        auto ShaderModuleVert = bd->ShaderModuleVert;
        g_device->defer_release([=](){
...
void ImGui_ImplVulkanH_DestroyFrameRenderBuffers(VkDevice device, ImGui_ImplVulkanH_FrameRenderBuffers* buffers, const VkAllocationCallbacks* allocator)
{
    ...
    {
        g_device->defer_release([device, buffer = buffers->VertexBuffer, allocator]() mutable{
            vkDestroyBuffer(device, buffer, allocator); buffer = VK_NULL_HANDLE;
        });
    }
    if (buffers->VertexBufferMemory) 
    {
        g_device->defer_release([device, buffer = buffers->VertexBufferMemory, allocator]() mutable{
            vkFreeMemory(device, buffer, allocator); buffer = VK_NULL_HANDLE;
        });
    }
    if (buffers->IndexBuffer) 
    {
        g_device->defer_release([device, buffer = buffers->IndexBuffer, allocator]() mutable{
            vkDestroyBuffer(device, buffer, allocator); buffer = VK_NULL_HANDLE;
        });
    }
    if (buffers->IndexBufferMemory) 
    {
        g_device->defer_release([device, buffer = buffers->IndexBufferMemory, allocator]() mutable{
            vkFreeMemory(device, buffer, allocator); buffer = VK_NULL_HANDLE;
        });
    }
            ...
            if (pcmd->TextureId)
            {
                ...
                if (findResult == lDescriptorSets.at(frameIndex).end())
                {
                    ...
                    if (vkAllocateDescriptorSets(v->Device, &alloc_info, &set) == VK_SUCCESS)
                        ...
                        DS_LOG_ERROR("Failed to allocate descriptor set");
                }
